...
export const loginScreenAction = async ({request}) => {
  ...
  const {user, error} = await
    (formId === "login" ? signInUser(email, password) : registerUser(username, email, password))
  if (user) {
  ...
  return (
    <div className={"h-screen w-screen bg-blue-200 flex  justify-center items-center"}>
      <div className={"flex flex-col"}>
        {/*form*/}
        <div className={"bg-white w-96 h-96 rounded-t-2xl flex flex-col justify-center items-center"}>
        ...
        <div className={"flex"}>
          ...
          <LoginScreenTab text={"Register"} onClick={switchToRegister} active={!isLogin}/>
        </div>
      </div>
    </div>
  )
